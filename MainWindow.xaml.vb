...
Class MainWindow
    ...
    Private Sub GetUpdatePackage()
        If IsURLValid("https://raw.githubusercontent.com/SvenGDK/OrbisPro/main/Update.zip") Then

            Dim NewWebClient As New WebClient()
            NewWebClient.DownloadFileAsync(New Uri("https://raw.githubusercontent.com/SvenGDK/OrbisPro/main/Update.zip"), My.Computer.FileSystem.CurrentDirectory + "\Update.zip", Stopwatch.StartNew)
            ...
            AddHandler NewWebClient.DownloadFileCompleted, Sub(sender As Object, e As AsyncCompletedEventArgs)
                                                           ...
                                                           End Sub
        Else
            MsgBox("Error while updating OrbisPro. Description: Could not find the latest Update package." + vbCrLf + "Please check your internet connection.", MsgBoxStyle.Critical, "Error while updating")
            Close()
        End If
    End Sub
    ...
    Private Sub InstallUpdate()
        If File.Exists(My.Computer.FileSystem.CurrentDirectory + "\Update.zip") Then
            ...
            Catch ex As Exception
                MsgBox("Error while updating OrbisPro. Description: " + ex.Message, MsgBoxStyle.Critical, "Error updating OrbisPro")
                Close()
            End Try

            UpdateTextBlock.Text = "Update installed successfully. OrbisPro will start in 2 sec."
            UpdateProgressBar.Value += 75

            FinishWaitTimer.Start()
        Else
            MsgBox("Error while updating OrbisPro. Description: " + vbCrLf + "File " + vbCrLf + My.Computer.FileSystem.CurrentDirectory + "\Update.zip" + vbCrLf + "not found.", MsgBoxStyle.Critical, "Error while updating")
            Close()
        End If
    End Sub

    Private Sub StartOrbisPro()
        If File.Exists(My.Computer.FileSystem.CurrentDirectory + "\OrbisPro.exe") Then
            Process.Start(My.Computer.FileSystem.CurrentDirectory + "\OrbisPro.exe")
        End If
        Close()
    End Sub
